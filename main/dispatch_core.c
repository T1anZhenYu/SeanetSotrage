    ...
    unsigned socket_id, uint8_t port_id)
{
    ...
    if (ipv6_hdr->proto == SEANET_PROT) {
        id_hdr = (struct seanet_hdr*)RTE_PTR_ADD(ipv6_hdr, sizeof(struct ipv6_hdr));
        ...
        if ((ntohs(udp_hdr->src_port) == MOB_EVENT_SRC_PORT) && (ntohs(udp_hdr->dst_port) == MOB_EVENT_DST_PORT)) {
            m_packet->ol_flags = MOBILE_TYPE;
            ring_name = get_rx_queue_name(app_conf.lcore_configuration.mobile, MB_2_DISPATCH_RECV_RING_NAME_FLAG);
            ring = rte_ring_lookup(ring_name);
            if (ring == NULL) {
                rte_exit(EXIT_FAILURE,
                    " core:socket %u has problem getting recv ring, ring_name:%s, "
        ...
        } else if ((ntohs(udp_hdr->src_port) == MULTICAST_SRC_PORT) && (ntohs(udp_hdr->dst_port) == MULTICAST_DST_PORT)) {
        ...
        } else if ((ntohs(udp_hdr->dst_port) < RESOLUTION_REPLY_UDP_PORT_MAX) && ntohs(udp_hdr->dst_port) >= RESOLUTION_REPLY_UDP_PORT_MIN) {
