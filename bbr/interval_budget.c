...
void set_target_rate_kbps(interval_budget_t* budget, int target_rate_kbps)
{
	...
	sim_debug(" func: %s, line:%d, bytes_remaining:%d,max_bytes_in_budget:%d\n",__FUNCTION__,__LINE__,budget->bytes_remaining,budget->max_bytes_in_budget);
}

void increase_budget(interval_budget_t* budget, int delta_ts)
{
	int bytes = budget->target_rate_kbps * delta_ts / 8;
	sim_debug(" func: %s, line:%d, bytes:%d,time:%d\n",__FUNCTION__,__LINE__,bytes,delta_ts );
...
void use_budget(interval_budget_t* budget, int bytes)
{
	...
	sim_debug(" func: %s, line:%d, bytes:%d,bytes_remaining:%d,max_bytes_in_budget:%d\n",__FUNCTION__,__LINE__,bytes,budget->bytes_remaining,budget->max_bytes_in_budget);
}
